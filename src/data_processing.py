...
def process_data(nome_area_risco, sigla_area):
    ...
    if not dados_fichas_uma_area.empty:
        # Crianças
        ...
        empty_gdf = pd.concat([new_gdf[['COR', 'RHD_SETOR']], pd.DataFrame(columns=['GPS_LAT_FICHA', 'GPS_LONG_FICHA'])])
        geometria_criancas = empty_gdf
        geometria_idoso = empty_gdf
        geometria_pcds = empty_gdf
        geometria_probl = empty_gdf
    ...
    if not dados_fichas_uma_area.empty:
        ...
        uso_csa = pd.DataFrame()
        tipologia_csa = pd.DataFrame()
        npavmto_csa2 = pd.DataFrame()
        acab_csa = pd.DataFrame()
        piso_csa = pd.DataFrame()
        telhado_csa = pd.DataFrame()
        probl_csa = pd.DataFrame()
